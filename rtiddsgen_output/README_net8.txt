...
This C# example was generated for the platform net8,
using the data type A3MPDataMsg from A3_MP.idl.

This example builds one application (A3_MPProgram.cs)
which can run a publisher example (A3_MPPublisher.cs)
or a subscriber example (A3_MPSubscriber.cs).
...
To modify the data being sent: edit the A3_MPPublisher.cs
file where it says
...
Open A3_MP.csproj with Visual Studio to edit, compile, debug,
and run the project.
...
You can also configure this package source globally:
    ...
    $ dotnet nuget add source C:\Program Files\rti_connext_dds-7.3.0\bin\../lib/dotnet -n RTI
...
2) In A3_MPProgram.cs remove the code that uses System.CommandLine.
